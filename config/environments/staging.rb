Rails.application.configure do
  ...
  # Rails.application.routes.default_url_options[:host] = 'hirepool.io'
  ...
  config.action_mailer.default_url_options = { host:'hirepool.io' }
  ...
  config.x.dynamodb.companies_table = 'companies-staging'

  # Defines which dynamodb table to use for saving company_glassdoor ratings information
  config.x.dynamodb.company_glassdoor_table = 'company-ratings-staging'

  # Defines which dynamodb table to use for saving User Surveys
  config.x.dynamodb.user_surveys_table = 'user-surveys-staging-encrypted'

  # Defines which dynamodb table to use for saving Premium Surveys
  config.x.dynamodb.premium_surveys_table = 'premium-surveys-staging-encrypted'

  # Defines which dynamodb table to use for saving FAQ questions
  config.x.dynamodb.faq_questions_table = 'faq-questions-staging'

  # Defines which lambda function to trigger for fetching interview reviews from glassdoor
  config.x.lambda.interview_reviews_job = 'interview_reviews_job_staging'

  # Defines web app URL used by DeviseTokenAuth initializer / config
  config.web_app_base_url = 'https://staging.hirepool.io/app/#'
end
