class PremiumSurvey
  ...
  @@premium_surveys_table = Rails.configuration.x.dynamodb.premium_surveys_table
  ...
  def save
    create_dynamodb_table(@@premium_surveys_table)
    begin
      @dynamo_client.put_item(:table_name => @@premium_surveys_table, :item => @dynamoItem)
  ...
  def load(user_id)
    resp = @dynamo_client.query({
      table_name: @@premium_surveys_table,
